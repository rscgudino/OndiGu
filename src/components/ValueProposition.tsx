...
export const ValueProposition: React.FC = () => {
  ...
  return (
    <section id="propuesta" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#0f1218] border-t border-b border-[#1e222e]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {VALUE_DIFFERENTIATORS.map((diff, index) => {
            ...
            return (
              <div
                key={diff.title}
                className="relative bg-[#151821] p-8 border border-[#232734] hover:border-[#323748] transition-colors flex flex-col justify-between"
              >
                {/* Accent indicator line on the top edge */}
                <div 
                  className="absolute top-0 left-0 w-12 h-[2px] bg-gradient-to-r from-[#FF4500] to-[#FF8C00]"
                  aria-hidden="true" 
                />

                <div>
                  <div className="flex items-center justify-between mb-6">
                    <span className="text-xs font-semibold tracking-wider text-[#FF8C00] bg-[#FF8C00]/10 px-2.5 py-1 rounded">
                      {diff.highlight}
                    </span>
                    <div className="w-9 h-9 flex items-center justify-center bg-[#1d202b] border border-[#2b3040] text-[#e0e0e0] rounded">
